...
function patchHistoryEvents() {
  ...
  if (win.__vaHistoryPatched) return;
...
const pendingMarkerX = computed(() => {
  if (!pendingPosition.value) return 0;
...
watch(
  ...
  (v) => {
    if (v) settings.blockPageInteractions = v;
  },
  { immediate: true }
);
watch(
  ...
  (v) => {
    if (v) settings.autoHideToolbar = v;
  },
  { immediate: true }
);
watch(
...
function onOverlayMouseUp(e: MouseEvent) {
  ...
  if (!el || isInsideAgentationTree(el)) return;
...
function syncUrlScope(nextUrl: string) {
  if (!nextUrl || currentUrl.value === nextUrl) return;
  dismissUndo();
...
function onDocumentMouseMove(e: MouseEvent) {
  if (!shouldUseDocumentFallbackEvents()) return;
...
function onDocumentClick(e: MouseEvent) {
  ...
  if (!target || isInsideAgentationTree(target, e)) return;
  e.preventDefault();
  e.stopPropagation();
}
...
function onUndo() {
  ...
  dismissUndo();
  if (snapshot.length > 0) restoreAnnotations(snapshot);
}
...
function onInputSave(comment: string) {
  ...
  if (updated) emit("annotation-update", updated);
  resetPendingState();
  transition("inspect");
}
