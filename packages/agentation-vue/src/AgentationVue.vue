<template>
  <component :is="portalWrapper" v-bind="portalProps">
    <div ref="rootEl" data-agentation-vue :data-va-theme="settings.theme !== 'auto' ? settings.theme : undefined">
      ...
      <!-- Copy feedback -->
      <div v-if="copyFeedback" class="__va-copy-feedback">Copied!</div>
...
import { ref, watch, onMounted, onBeforeUnmount, h, defineComponent } from 'vue-demi'
...
import { detectVueComponents, isFixed as checkIsFixed, getNearbyElements, getComputedStylesSummary, getAccessibilityInfo } from './utils/dom-inspector'
...
const { hoveredElement, hoveredRect, hoveredName, hoveredComponentChain, onMouseMove, clearHighlight, getElementUnderOverlay, cleanup: cleanupDetection } = useElementDetection(overlayEl, () => settings.showComponentTree)
...
const { recalculatePositions } = useMarkerPositions(annotations)
...
watch(() => props.outputDetail, (v) => { if (v) settings.outputDetail = v as any }, { immediate: true })
watch(() => props.markerColor, (v) => { if (v) settings.markerColor = v }, { immediate: true })
watch(() => props.theme, (v) => { if (v) settings.theme = v }, { immediate: true })
...
function onOverlayMouseDown(e: MouseEvent) {
  if (multiSelect.onMouseDown(e)) return
  if (areaSelect.onMouseDown(e)) return
}
...
function onOverlayMouseUp(e: MouseEvent) {
  ...
  if (!el || el.closest('[data-agentation-vue]')) return
...
function onMarkerClick(ann: Annotation) {
  ...
  if (removed) emit('annotation-delete', removed)
}
